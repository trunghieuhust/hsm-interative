...
public class DistrictStatisticPane extends JPanel {
	...
	public DistrictStatisticPane() {
		...
		JLabel lblNumberOfLecturers = DefaultComponentFactory.getInstance().createLabel("Number of lecturers");
		...
		JLabel lblNumberOfStudents = DefaultComponentFactory.getInstance().createLabel("Number of students");
	...
	protected void refresh() {
		if (district == null)
			return;
		String[] rr = (String[]) Control.getInstance().getData("getSingleDistrictStatistic", district);
		if (rr == null){
			JOptionPane.showMessageDialog(Control.getInstance().getMainWindow(), "Refresh district's statistic information failed\nMessage: " + Control.getInstance().getQueryMessage(), "Refresh faileD", JOptionPane.WARNING_MESSAGE);
			return;
		}
	...
	public void export(HedspiTable hedspiTable) {
		hedspiTable.addValue("Number of lecturers", textFieldNLecturers.getText());
		hedspiTable.addValue("Number of students", textFieldNStudents.getText());
