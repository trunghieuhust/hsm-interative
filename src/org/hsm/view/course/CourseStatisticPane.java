...
public class CourseStatisticPane extends JPanel {
	...
	public CourseStatisticPane() {
		...
		JLabel lblTeachedLecturers = DefaultComponentFactory.getInstance()
				.createLabel("Teached lecturers");
		...
		JLabel lblPassedStudents = DefaultComponentFactory.getInstance()
				.createLabel("Passed students");
		...
		JLabel lblFailedStudents = DefaultComponentFactory.getInstance()
				.createLabel("Failed students");
		...
		JLabel lblBestPoint = DefaultComponentFactory.getInstance()
				.createLabel("Best point");
		...
		JLabel lblWorstPoint = DefaultComponentFactory.getInstance()
				.createLabel("Worst point");
	...
	protected void refresh() {
		if (course == null)
			return;
		String[] val = (String[]) Control.getInstance().getData(
				"getSingleCourseStatistic", course);
		if (val == null) {
			JOptionPane.showMessageDialog(
					Control.getInstance().getMainWindow(),
					"Get course statistic failed\nMessage: "
	...
	public void export(HedspiTable hedspiTable) {
		hedspiTable.addValue("Number of teached lecturers",
				textFieldLecturers.getText());
		hedspiTable.addValue("Number of passed students",
				textFieldPassedStudents.getText());
		hedspiTable.addValue("Number of failed students",
				textFieldFailedStudents.getText());
