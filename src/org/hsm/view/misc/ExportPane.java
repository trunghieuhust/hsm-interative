...
public class ExportPane extends JPanel {
	 ...
	 */
	public ExportPane() {
		setLayout(new MigLayout("", "[][][grow][][][grow]", "[][][][][][][][49.00,grow][][][][][][][]"));
		...
		final JComboBox<Object> comboBox_exportObj = new JComboBox<Object>(exportObject);
		add(comboBox_exportObj, "cell 2 1 4 1,growx");
		
		JLabel lblExportFields = new JLabel("Export fields");
		add(lblExportFields, "cell 0 2");
		
		final JCheckBox chckbxPersonalDetails = new JCheckBox("Personal details");
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				// html_content is whole content to write to .html
						if (comboBox_exportObj.getSelectedIndex() == 0) {
							...
							html_content += "</tr>\n";
							// If "Export All" is not selected, check out list of student
							if (!chckbxAll.isSelected()){
								...
								for (int i = 0; i < parts.length; i++) {
									ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_student_by_mssv", parts[i]);
									...
									if (chckbxInfo.isSelected()) {
										...
										html_content += "<td>" + hedspiClass + "</td>\n";
									}
									//Contact
									if (chckbxContact.isSelected()) {
										//emails
										...
										if (ret.get("dt") == null)
											...
											dt= (int) ret.get("dt");
										//System.out.print((String) Model.getInstance().getData("getDistrictName", dt));
										html_content += "<td>" + (String) Model.getInstance().getData("getDistrictName", dt) + "</td>";
										//home
								...
								ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_all_students");
								for (int i = 0; i < rs.size(); i++) {
										...
										if (chckbxContact.isSelected()) {
											...
											if (ret.get("dt") == null)
												...
												dt= (int) ret.get("dt");
											html_content += "<td>" + Model.getInstance().getData("getDistrictName", dt) + "</td>";
											//home
											String home = (String) ret.get("home");
											if (home == null)
												home = "";
						...
						if (comboBox_exportObj.getSelectedIndex() == 1) {
							...
							if (!chckbxAll.isSelected()){
								...
								// Search for all student code in list
								for (int i = 0; i < parts.length; i++) {
									ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_lecturer_by_fullname", parts[i]);
									HashMap<String, Object> ret = rs.get(0);
									html_content += "<tr>";
									if (chckbxPersonalDetails.isSelected()) {
										//Fullname
								...
								// If "Export all" checked, export list of all student
								ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_all_lecturers");
								for (int i = 0; i < rs.size(); i++) {
										HashMap<String, Object> ret = rs.get(i);
										html_content += "<tr>";
										if (chckbxPersonalDetails.isSelected()) {
						...
						if (comboBox_exportObj.getSelectedIndex() == 2) {
							...
							if (!chckbxAll.isSelected()){
								...
								String[] parts = raw_list.split(";");
								for (int i = 0; i < parts.length; i++) {
									ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_course_by_code", parts[i]);
									HashMap<String, Object> ret = rs.get(0);
									html_content += "<tr>";
									//coursename
								...
								ArrayList<HashMap<String, Object>> rs = CoreService.getInstance().doQueryFunction("get_all_courses");
								for (int i = 0; i < rs.size(); i++) {
										HashMap<String, Object> ret = rs.get(i);
										html_content += "<tr>";
										//coursename
